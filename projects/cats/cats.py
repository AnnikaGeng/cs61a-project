...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    kps = list(x for x in paragraphs if select(x) == True)
    if k <= len(kps)-1:
        return kps[k]
    else:
        return ''
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    lower_topic = [lower(x) for x in topic]
    def whether(ps):
        ps = split(remove_punctuation(lower(ps)))
        w = False
        for word in lower_topic:
            for p in ps:
                if word == p:
                    w = True
        return w
...
def accuracy(typed, reference):
    ...
    def compare(typed_words, reference_words):
        result = 0.0
        count = 0
        length_r = len(reference_words)
        length_t = len(typed_words)
        if length_t > length_r:
            typed_words = typed_words[0:length_r]
            ...
            for i in range(length_t):
                if typed_words[i] == reference_words[i]:
                    count += 1
        result = (count / length_t) * 100
        return result
    return compare(typed_words, reference_words)
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    length_by_group = len(typed) / 5
    minute = elapsed / 60
    return length_by_group / minute
    # END PROBLEM 4
    ...
    if typed_word in valid_words:
        ...
        for word in valid_words:
            ...
            if diff_result <= limit:
                group.append([word,diff_result])
        if group == []:
            return typed_word
        else:
            return min(group, key=lambda diff: min(diff[1:]))[0]
...
def feline_flips(start, goal, limit):
    ...
    count = abs(len(goal) - len(start))
    i = min(len(goal),len(start)) - 1
    def recurse(i, count):
        if count > limit:
            return limit + 1
        else:
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(len(sofar)):
        if sofar[i] == prompt[i]:
            ...
            break
    upload({'id': user_id, 'progress': k/len(prompt)}) 
    return k/len(prompt)
...
def time_per_word(words, times_per_player):
    ...
    result = []
    for group in times_per_player:
        result.append(subtract(group))
...
def fastest_words(match):
    ...
    word = get_words(match)
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    def zip_the_list(lists):
        return list(zip(*lists))

    def constract_empty_list(list):
        empty_list = []
        for i in list:
            empty_list.append([])
        return empty_list
    
    def find_min_indice(list):
        min = list[0]
        min_indice = 0
        k = 0
        while k < len(list):
    ...
    def result_list(lists):
        pair_list = zip_the_list(lists)
        result = []
        for list in pair_list:
            indice = find_min_indice(list)
            result.append(indice)
        return result
    
    def render_the_result(origin,result,words):
        empty = constract_empty_list(origin)
        for i in range(len(result)):
            num = result[i]
            empty[num].append(words[i])
        return empty


    min_indice_list = result_list(get_times(match))
    origin = get_times(match)
    return render_the_result(origin,min_indice_list,word)
    # END PROBLEM 10
